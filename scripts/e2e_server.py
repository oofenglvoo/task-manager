...
`CreateProcessW` 传递；把三个标准句柄重定向（stdin=DEVNULL，stdout/stderr
到文件），父进程就不会因为管道/句柄不关闭而阻塞。

**控制台窗口（踩过两次的坑）**：本脚本自身运行在没有可继承控制台的环境里，
所以每个控制台子系统子进程（python / powershell / taskkill / netstat）都必
须带 `NO_WINDOW`（`CREATE_NO_WINDOW`），否则会被 Windows 新分配一个**可见的
空白控制台窗口**。

但注意 `Popen` 启动 uvicorn 这一处的特殊性：**绝对不能用
`DETACHED_PROCESS`**。`DETACHED_PROCESS` 会把子进程从调用者的控制台**完全
脱离**（连伪终端都不给），而控制台子系统（Subsystem=3）的 `python.exe` 在
无控制台可用、标准句柄又是真实句柄时，Windows 会拉起**默认终端应用**
（本机是 Windows Terminal）为它**新建一个终端窗口/标签页**——窗口类名
`CASCADIA_HOSTING_WINDOW_CLASS`，标题就是子进程的 exe 路径，看起来像「空白
的 PowerShell 窗口」。此时 `CREATE_NO_WINDOW` **会被忽略**（实测：
`NO_WINDOW` 单独用干净；`DETACHED | NO_WINDOW` 和 `DETACHED` 单独用都弹窗），
所以两者叠加也救不回来。

正确做法是只用
`CREATE_NEW_PROCESS_GROUP | NO_WINDOW`：子进程拿到一个**不可见**的新控制台，
句柄已重定向到文件，父进程立即返回且不阻塞。判定方法是枚举顶层窗口看是否
新增 `CASCADIA_HOSTING_WINDOW_CLASS`（见 AGENTS.md 的 E2E 小节）。
...
def start(port: int, keep_db: bool) -> int:
    ...
    creationflags = 0
    if os.name == "nt":
        # 只用 NO_WINDOW：给子进程一个不可见的新控制台。三个标准句柄都已重定向，
        # 父进程立即返回、不会被句柄拖住。
        # 不要加 DETACHED_PROCESS：它会让子进程无控制台可用，Windows 随即拉起
        # 默认终端应用新建一个终端窗口（CASCADIA_HOSTING_WINDOW_CLASS，标题是
        # exe 路径），且此时 NO_WINDOW 会被忽略 —— 弹窗就是这么来的。
