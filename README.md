...
- **外观设置**：深色 / 浅色 / 跟随系统主题（顶栏可快捷切换）、卡片大小（小/中/大）、显示密度（宽松/紧凑）、自定义背景图片（本地上传或图片链接）、**背景遮罩与卡片/面板透明度**（三个滑块）；偏好保存在后端并在浏览器缓存
...
| GET/PUT | `/api/settings` | 外观偏好（主题 / 卡片大小 / 紧凑模式 / 分组视图 / 背景 URL / 背景遮罩与卡片、面板透明度） |
