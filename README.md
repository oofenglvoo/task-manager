...
- **日历视图**：**月视图 / 周视图**切换、翻页、回到今天；每格显示公历日、**农历（阴历）/节气/传统节日**与法定节假日**「休 / 班」角标**，并以**当天任务数（完成/总数）与完成度着色**体现进度（全部完成绿 / 进行中黄 / 含逾期红 / 待开始蓝），逾期任务按时间精确判定；点击某天弹出当天任务列表，可直接打开任务详情或**新建任务（预填该日截止时间）**；复用顶部筛选（优先级/标签/搜索），默认排除已归档任务，周一为一周起始
